...
export class EnemyControllers extends Component {
    ...
    diedReward() {
        ...
        let decision_re_capability = GameData.userData.decision_re_capability;
        //this.money_drop.active = true;
        let reward_num: number = 0;
        let str: string
        //boss掉落奖励
        if (this.data.is_boss) {
            reward_num = this.reward.boss_reward + this.reward.normal_reward + revenue_capability + decision_re_capability;
            str = '加' + reward_num;
        } else {//普通奖励
            reward_num = this.reward.normal_reward + revenue_capability + decision_re_capability;
            str = '加' + reward_num;
        }
