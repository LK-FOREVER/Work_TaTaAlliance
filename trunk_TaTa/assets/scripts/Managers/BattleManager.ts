...
export class BattleManager extends Component {
    ...
    ctorMap() {
        //不再生效
        this.isfirstAllHurt = true;
        this.isfirstAllStop = true;
        this.coolingTime();            //技能冷却

        let decisionAddNum = GameData.userData.decision_re_capability;
        let allAddNum = GameData.userData.chapter + GameData.userData.revenue_capability + decisionAddNum;
        this.node.getChildByName("battle_desc").getChildByName("NowLvFund").getComponent(Label).string = `本关资金掉落：${allAddNum}`;
        this.node.getChildByName("battle_desc").getChildByName("Foundation").getComponent(Label).string = `基础掉落：${GameData.userData.chapter}`;
        this.node.getChildByName("battle_desc").getChildByName("Capacity").getComponent(Label).string = `营收能力：${GameData.userData.revenue_capability}`;
        this.node.getChildByName("battle_desc").getChildByName("Decision").getComponent(Label).string = `决策加成：${decisionAddNum}`;
    ...
    //全体禁锢和全体伤害技能冷却
    coolingTime() {
        this.node.getChildByName("btn_all_stop").getChildByName("mask").active = !this.isfirstAllStop;
        this.node.getChildByName("btn_all_atk").getChildByName("mask").active = !this.isfirstAllHurt;
    }

    //判断是否开启决策
    initDecision() {
        let chapter = GameData.userData.chapter;
        GameData.userData.decision_re_capability = 0;
        this.decision_start_view = this.node.getChildByName("decision_start_view");
        this.decision_view = this.node.getChildByName("decision_view");
        this.decision_removebuff_view = this.node.getChildByName("decision_removebuff_view");
        this.decision_removebuff_view.active = false;
        let decision_num = GameData.userData.decisionList.length;
        //boss关，并且决策选中15个，开启决策
        if (chapter >= 10 && chapter % 5 == 0 && decision_num == 15) {
            this.setDecisionStartView(true);
            this.is_start = false;
        } else {
            this.is_start = true;
        }
    ...
    //显示隐藏决策时机界面
    setDecisionStartView(is_show: boolean) {
        if (is_show) {
            this.decision_start_view.active = true;
            this.is_start = false;
        } else {
            this.decision_start_view.active = false;
        }
    }
    //显示隐藏决策界面
    setDecisionView(is_show: boolean) {
        if (is_show) {
            this.decision_view.active = true;
            let dicisionctor = this.decision_view.getComponent(DecisionController);
            dicisionctor.init();
        } else {
            this.decision_view.active = false;
        }
    }
    //显示隐藏决策消除buff界面
    setDecisionRemoveBuff(is_show: boolean, negativeResult: any | null) {
        if (is_show) {
            this.decision_removebuff_view.active = true;
            let dicisionbuffctor = this.decision_removebuff_view.getComponent(
                DecisionBuffController
            );
            dicisionbuffctor.init(negativeResult);
        } else {
            if (this.decision_removebuff_view.active) {
                this.decision_removebuff_view.active = false;
            }
            this.is_start = true;
            //决策开始战斗
