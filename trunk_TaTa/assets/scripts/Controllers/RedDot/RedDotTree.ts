import { RedDotNode, RedDotCallback } from "./RedDotNode";

export class RedDotTree {

    private rootNode = new RedDotNode("", true);
     ...
     */
    public addNode(redDotPath: string, isNum: boolean = true): RedDotNode {
    ...
    public registerCallback(redDotPath: string, callback: RedDotCallback): boolean {
        ...
        if(!node.setCallback(callback)) {
            RedDotNode.logError(`RedDotTree: ${redDotPath} override existing callback failed:`)
            return false;
        }
        return true;
    }
