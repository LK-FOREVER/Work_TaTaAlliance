...
export class RedDotNode {
    ...
    public static logError(...data: any[]) {
        if(RedDotNode.errorFucntion) {
            this.errorFucntion(data);
        }
    }

    public readonly name: string;
    private children: Map<string, RedDotNode> = new Map<string, RedDotNode>();
    private parent: RedDotNode = null;
    ...
    public set redNum(newValue: number) {
        if(this.children.size > 0) {
            ...
            RedDotNode.logError(`RedDotTree: change the value of non leaf node is not allowed! ${this.name}`);
            return;
        }
    ...
    public addNode(subPath: string, isNum: boolean): RedDotNode {
        let names = subPath.split("/");
        let parent: RedDotNode = this;
        for (let index = 0; index < names.length - 1; index++) {
            ...
            if(!parent) {
                ...
                RedDotNode.logError(`RedDotTree: can't find parent ${name} of path ${subPath}`);
                return null;
            }
        }
        ...
        if(parent.children.get(childName)) {
            RedDotNode.logError(`RedDotTree: can't add the same node twice: ${subPath}`);
            return null;
        }
        let newNode = new RedDotNode(childName, isNum);
        parent.children.set(childName, newNode);
        newNode.parent = parent;
        return newNode;
    }

    public getNode(subPath: string): RedDotNode {
        let names = subPath.split("/");
